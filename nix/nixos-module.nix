...
in
{
  options.programs.headroom = {
    ...
    autoStart = mkOption {
      ...
      description = ''
        ...
        `default.target` so it autostarts without `systemctl
        --user enable headroom`.

        `systemd.packages` installs the unit but doesn't process its
        `[Install]` section, so the `default.target.wants`
  ...
  config = mkIf cfg.enable {
    ...
    # emits the default.target.wants symlink. unit body comes from the
