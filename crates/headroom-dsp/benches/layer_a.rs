...
fn bench_filter_kernels(c: &mut Criterion) {
    ...
    let mut silence_lim = Limiter::new(LimiterConfig::default(), SR);
    let mut block_lim = Limiter::new(LimiterConfig::default(), SR);
    let mut silence_block_lim = Limiter::new(LimiterConfig::default(), SR);
    let block = make_block();
    let mut silence = vec![0.0_f32; FRAMES * CHANNELS];
    ...
    group.bench_function("limiter_process_frame_silence", |b| {
        b.iter(|| {
            let (l, r) = silence_lim.process_frame(black_box(0.0), black_box(0.0));
            black_box((l, r));
        });
    });
    group.bench_function("limiter_process_block_1024", |b| {
        b.iter_batched(
            || block.clone(),
            |mut buf| {
                block_lim.process_interleaved_stereo(black_box(&mut buf));
                black_box(buf);
            },
            BatchSize::SmallInput,
        );
    });
    group.bench_function("limiter_process_block_silence_1024", |b| {
        b.iter(|| {
            silence_block_lim.process_interleaved_stereo(black_box(&mut silence));
            black_box(&silence);
        });
    });
    group.finish();
