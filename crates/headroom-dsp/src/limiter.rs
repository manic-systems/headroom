...
const SILENCE_EPS: f32 = 1.0e-20;
const OUTPUT_IDLE_EPS: f32 = 1.0e-12;
const GAIN_IDLE_EPS: f32 = 1.0e-4;
...
pub struct Limiter {
    ...
    last_hard_gr_db: f32,
    last_output_lin: f32,
    idle_drain_frames: usize,
    silent_drain_remaining: usize,
}
...
impl Limiter {
    ...
    pub fn new(cfg: LimiterConfig, sample_rate: f32) -> Self {
        ...
        let lookahead_frames = lookahead_samples_os.div_ceil(os);
        let idle_drain_frames = lookahead_frames + cfg.fir_taps * 2 + 2;
    ...
    pub fn process_frame(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        let input_silent = left.abs() <= SILENCE_EPS && right.abs() <= SILENCE_EPS;
        if input_silent && self.signal_path_idle() {
            self.last_peak_lin = 0.0;
            self.last_gr_db = 0.0;
            self.last_soft_gr_db = 0.0;
            self.last_hard_gr_db = 0.0;
            self.last_output_lin = 0.0;
            return (0.0, 0.0);
        }
        if input_silent {
            self.silent_drain_remaining = self.silent_drain_remaining.saturating_sub(1);
        } else {
            self.silent_drain_remaining = self.idle_drain_frames;
        }
        ...
        self.last_output_lin = out_l.abs().max(out_r.abs());
        if input_silent
            && self.silent_drain_remaining == 0
            && self.last_output_lin <= OUTPUT_IDLE_EPS
            && self.gain_state_idle()
        {
    ...
    fn signal_path_idle(&self) -> bool {
        self.silent_drain_remaining == 0
            && self.last_peak_lin <= OUTPUT_IDLE_EPS
            && self.last_output_lin <= OUTPUT_IDLE_EPS
            && self.gain_state_idle()
    }

    fn gain_state_idle(&self) -> bool {
        self.hold_remaining == 0
            && (self.hard_gain - 1.0).abs() <= GAIN_IDLE_EPS
            && self
                .soft_envelope
                .as_ref()
                .is_none_or(|env| (env.state() - 1.0).abs() <= GAIN_IDLE_EPS)
    }
}
...
mod tests {
    ...
    #[test]
    fn silence_fast_path_preserves_program_loudness() {
        let sr = 48_000.0;
        let mut l = Limiter::new(LimiterConfig::default(), sr);
        l.set_program_loudness_lufs(-18.0);
        let before = l.effective_soft_ceiling_dbtp().unwrap();

        for _ in 0..10_000 {
            let (lo, ro) = l.process_frame(0.0, 0.0);
            assert_eq!(lo, 0.0);
            assert_eq!(ro, 0.0);
        }

        assert_eq!(l.effective_soft_ceiling_dbtp().unwrap(), before);
        assert_eq!(l.gain_reduction_db(), 0.0);
        assert_eq!(l.true_peak_dbtp(), -200.0);
    }

    #[test]
    fn silence_fast_path_does_not_truncate_lookahead_tail() {
        let sr = 48_000.0;
        let mut l = Limiter::new(LimiterConfig::brickwall_only(), sr);
        for _ in 0..16 {
            let _ = l.process_frame(0.5, 0.5);
        }
        ...
        for n in 0..200 {
            let (lo, ro) = l.process_frame(0.0, 0.0);
            if n > 8 {
                delayed_peak = delayed_peak.max(lo.abs()).max(ro.abs());
            }
        }

        assert!(
            delayed_peak > 1.0e-6,
            "lookahead tail was truncated before it could drain: delayed_peak={delayed_peak}"
        );
    }
