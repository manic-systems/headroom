...
impl Compressor {
    ...
    pub fn process_frame(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        if left == 0.0 && right == 0.0 && self.envelope_db <= -199.0 && self.rms_state == 0.0 {
            self.envelope_db = -200.0;
            self.last_gr_db = 0.0;
            return (0.0, 0.0);
        }
...
mod tests {
    ...
    #[test]
    fn exact_zero_crossing_does_not_reset_active_envelope() {
        let cfg = CompressorConfig {
            threshold_db: -24.0,
            attack_ms: 0.1,
            release_ms: 200.0,
        ...
        let mut c = Compressor::new(cfg, 48_000.0);
        for _ in 0..1_000 {
            let _ = c.process_frame(1.0, 1.0);
        }
        assert!(c.gain_reduction_db() < -10.0);

        let _ = c.process_frame(0.0, 0.0);
        assert!(
            c.gain_reduction_db() < -10.0,
            "a single exact-zero frame should release, not reset"
        );
    }
