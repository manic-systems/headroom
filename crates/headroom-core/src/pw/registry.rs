...
pub struct RoutingState {
    ...
    /// whether the processed-sink monitor -> filter -> real-sink graph
    /// should currently be linked.
    bus_graph_active: bool,
...
impl RoutingState {
    ...
    fn try_capture_real_sink(&mut self, global: &GlobalObject<&DictRef>) {
        ...
        if became_real_sink {
            ...
            self.write_policy_default_audio_sink();
    ...
    fn sync_bus_graph_links(&mut self) {
        let should_be_active = self.should_activate_bus_graph();
        if should_be_active {
            if !self.bus_graph_active {
                tracing::info!("processed bus graph activated");
            }
            self.bus_graph_active = true;
            self.enqueue_filter_input_link();
            self.pin_filter_to_real_sink();
        } else {
            self.drop_bus_graph_links();
            if self.bus_graph_active {
                tracing::info!("processed bus graph deactivated");
            }
            self.bus_graph_active = false;
        }
    }

    fn should_activate_bus_graph(&self) -> bool {
        let s = self.daemon.lock();
        if s.profiles.bypass_global() {
            return false;
        }
        if s.profiles.effective().default_route.route == Route::Processed {
            return true;
        }
        s.streams.values().any(|r| r.route == Route::Processed)
    }

    fn drop_bus_graph_links(&mut self) {
        let processed_id = { self.daemon.lock().processed_sink_id };
        if let Some(processed_id) = processed_id {
            self.pending_routes.remove(&processed_id);
            self.managed_route_links.remove(&processed_id);
        }
        if let Some(filter_id) = self.filter_playback_id {
            self.pending_routes.remove(&filter_id);
            self.managed_route_links.remove(&filter_id);
        }
    }
    ...
    fn reassert_default_processed(&mut self) {
        // under default-bypass policy, stop fighting WP for the default so
        // `default`-following apps land at the real sink — else
        // "bypass on" wouldn't bypass for them.
        if self.default_should_bypass() {
            return;
        }
    ...
    fn write_policy_default_audio_sink(&self) {
        if self.default_should_bypass() {
            let real_sink_name = { self.daemon.lock().real_sink.name.clone() };
            if let Some(name) = real_sink_name {
                tracing::info!(
                    sink = name.as_str(),
                    "bypass default: setting default.audio.sink to real sink"
                );
                self.write_default_audio_sink(&name);
            } else {
                tracing::warn!(
                    "bypass default requested but no real sink known yet — leaving default.audio.sink alone"
                );
            }
        } else {
            self.write_default_audio_sink(PROCESSED_SINK_NAME);
    ...
    fn default_should_bypass(&self) -> bool {
        let s = self.daemon.lock();
        s.profiles.bypass_global() || s.profiles.effective().default_route.route == Route::Bypass
    }
