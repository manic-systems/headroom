...
fn process_inner(state: &mut FilterState) {
    ...
    let measurement_dropped = push_measurements(
        &mut state.measurement_producer,
        in_l_samples,
        in_r_samples,
        frames,
    );
    ...
    if measurement_dropped > 0 {
        state.measurement_dropped = state
            .measurement_dropped
            .saturating_add(measurement_dropped);
    }
...
/// Feed the slow AGC in one ring reservation per callback. If the
/// controller is behind, keep the prefix that fits and drop the tail;
/// gaps are acceptable for loudness tracking and this never blocks.
fn push_measurements(
    ...
    let requested = frames.saturating_mul(CHANNELS as usize);
    if requested == 0 {
        return 0;
    }
    let writable = match producer.write_chunk(requested) {
        Ok(chunk) => {
            fill_measurement_chunk(chunk, left, right);
            return 0;
        }
        Err(rtrb::chunks::ChunkError::TooFewSlots(slots)) => slots & !(CHANNELS as usize - 1),
    };
    if writable == 0 {
        return requested as u64;
    }
    match producer.write_chunk(writable) {
        Ok(chunk) => {
            fill_measurement_chunk(chunk, left, right);
            (requested - writable) as u64
        }
        Err(_) => requested as u64,
    }
}

fn fill_measurement_chunk(
    mut chunk: rtrb::chunks::WriteChunk<'_, f32>,
    left: &[f32],
    right: &[f32],
) {
    let mut sample_idx = 0;
    let (first, second) = chunk.as_mut_slices();
    sample_idx = fill_measurement_slice(first, sample_idx, left, right);
    let _ = fill_measurement_slice(second, sample_idx, left, right);
    chunk.commit_all();
}

fn fill_measurement_slice(
    dst: &mut [f32],
    ...
    for slot in dst {
        let frame_idx = sample_idx / CHANNELS as usize;
        let sample = if sample_idx % CHANNELS as usize == 0 {
            left[frame_idx]
        } else {
            right[frame_idx]
        };
...
mod tests {
    ...
    fn apply_audio_cmd_skips_structural_limiter_change_silently() {
        ...
        assert_eq!(
            limiter.config().oversample,
            LimiterConfig::default().oversample
        );
    }
