...
const CONNECT_RETRY_TIMEOUT: Duration = Duration::from_secs(20);
...
impl PwContext {
    ...
    pub fn new() -> Result<Self, DaemonError> {
        ...
        let core = connect_with_retry(&context)?;
...
fn connect_with_retry(context: &Context) -> Result<Core, DaemonError> {
            ...
            Ok(core) => {
                if attempts > 1 {
                    tracing::info!(
                        ...
                        "pipewire connect succeeded after retry"
                    );
                }
                return Ok(core);
            }
            Err(e) if started.elapsed() < CONNECT_RETRY_TIMEOUT => {
                tracing::warn!(
                    attempts,
                    error = %e,
                    "pipewire connect failed; retrying"
                );
                std::thread::sleep(CONNECT_RETRY_INTERVAL);
            }
            Err(e) => {
                return Err(DaemonError::pipewire(format!(
                    "Context::connect after {attempts} attempts over {:?}: {e}",
                    started.elapsed()
                )));
            }
        }
