...
mod profile_effects;
use profile_effects::ProfileEffects;
...
fn setting_get(id: u64, key: &str, state: &SharedState) -> Response {
    ...
    let json_value = match serde_json::to_value(s.profiles.effective()) {
        Ok(v) => v,
        Err(e) => {
            return err(id, ErrorCode::Internal, format!("serialise profile: {e}"));
        }
    };
    drop(s);
...
fn profile_use(id: u64, name: &str, state: &SharedState) -> Response {
    ...
    match s.profiles.use_profile(name) {
        Ok(()) => {
            ...
            let post = ProfileEffects::profile_used(name).apply(&mut s);
            drop(s);
            post.finish();
...
pub(crate) fn execute_reload(
    ...
    let post = ProfileEffects::profile_reloaded(report.loaded.clone()).apply(&mut s);
    drop(s);
    post.finish();
    Ok(report)
}
...
fn route_set(id: u64, app: &str, to: Route, state: &SharedState) -> Response {
    ...
    match s.profiles.set_route(app, to) {
        Ok(()) => {
            ...
            let post = ProfileEffects::route_rule_changed().apply(&mut s);
            drop(s);
            post.finish();
            ok(id, &Value::Null)
        }
...
fn setting_set(id: u64, key: &str, value: Value, state: &SharedState) -> Response {
    ...
    match s.profiles.set_setting(key, value) {
        Ok(()) => {
            ...
            let post = ProfileEffects::settings_changed().apply(&mut s);
            drop(s);
            post.finish();
            ok(id, &Value::Null)
        }
...
fn route_stream(id: u64, node_id: u32, to: Route, state: &SharedState) -> Response {
    ...
    if let Some(tx) = tx {
            ...
            .is_err()
        {
            tracing::warn!(
                node_id,
                "PipeWire command channel closed; metadata write skipped"
            );
        }
    } else {
        tracing::debug!(
...
fn bypass_set(id: u64, enabled: bool, state: &SharedState) -> Response {
    ...
    match s.profiles.set_bypass(enabled) {
        Ok(()) => {
            ...
            let post = ProfileEffects::bypass_changed().apply(&mut s);
            drop(s);
            post.finish();
            ok(id, &Value::Null)
        }
...
fn per_app_set(id: u64, app: &str, enabled: bool, state: &SharedState) -> Response {
    ...
    match s.profiles.set_per_app_enabled(app, enabled) {
        Ok(()) => {
            ...
            let post = ProfileEffects::per_app_changed().apply(&mut s);
            drop(s);
            post.finish();
            ok(id, &Value::Null)
        }
...
fn layer_a_reset(id: u64, node_id: u32, state: &SharedState) -> Response {
    ...
    if let Some(tx) = tx {
            ...
            .is_err()
        {
            tracing::warn!(
                node_id,
                "PipeWire command channel closed; layer-a reset lost"
            );
        }
    } else {
        tracing::debug!(
            node_id,
            "no PipeWire command channel; layer-a reset skipped (test mode)"
        );
    }
...
fn store_err_to_response(id: u64, e: StoreError) -> Response {
    let code = match &e {
        ...
        StoreError::Io(_) | StoreError::OverlayParse(_) | StoreError::OverlaySerialize(_) => {
            ErrorCode::Internal
        }
    };
...
mod tests {
    ...
    fn status_surfaces_store_warnings() {
        ...
        assert!(
            warnings
                .iter()
                .any(|w| w.as_str().unwrap_or("").contains("broken.toml")),
    ...
    fn bypass_set_toggles_flag() {
        ...
        dispatch(&Request::new(1, Op::BypassSet { enabled: true }), &state);
        assert!(state.lock().profiles.bypass_global());

        dispatch(&Request::new(2, Op::BypassSet { enabled: false }), &state);
        assert!(!state.lock().profiles.bypass_global());
    }
    ...
    fn route_set_replaces_existing_user_rule() {
        ...
        let user_rules: Vec<_> = rules
            .iter()
            .filter(|r| r.match_.process_binary.len() == 1 && r.match_.process_binary[0] == "obs")
            .collect();
