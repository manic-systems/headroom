...
use headroom_ipc::{
    DaemonEvent, Event, LayerALevel, LayerASnapshot, MeterTick, Route, RoutingEvent, Status,
    StreamRoute, Topic,
};
use ratatui::{
...
mod tests {
    ...
    fn stream_removed_prunes_state() {
        ...
        assert!(!state.streams.contains_key(&7));
        assert!(!state.layer_a.contains_key(&7));
        assert!(!state.la_snapshots.contains_key(&7));
    }
